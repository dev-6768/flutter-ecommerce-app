...
class BuyNowSellerDetails extends StatefulWidget {
  const BuyNowSellerDetails({super.key});

  @override
  State<BuyNowSellerDetails> createState() => _BuyNowSellerDetailsState();
}

class _BuyNowSellerDetailsState extends State<BuyNowSellerDetails> {
  Future<List<String>> getDataForCurrentInstance() async{
    List<String> SellerDetailsList = await GetDataFromCloudFirestore.getRegistrationData(sellerUid);
      ...
      if(snapshot.connectionState == ConnectionState.done){
        if(snapshot.hasError){
          ...
          if(data[0] == "" || data[1] == "" || data[2] == "" || data[3] == "" || data[4] == "" || data[5] == "") {
                ...
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                        Center(
                        child: Text(
                          "Seller details not available at this moment.",
                          style: TextStyle(
                            fontFamily: GoogleFonts.lato().fontFamily,
                          ),
                          textScaleFactor: 1.5,
            ...
            const fontSizeForSellerText = 16.0;
            return Padding(
              padding: EdgeInsets.symmetric(vertical: 8.0, horizontal: 8.0),
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    "Name of the seller: ${data[0]}",
                    style: TextStyle(
                      fontFamily: GoogleFonts.lato().fontFamily,
                      fontSize: fontSizeForSellerText,
                    ),
                  ),
